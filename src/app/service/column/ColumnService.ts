...
export default class ColumnService extends BaseService {
    ...
    static URL_RICH_PAGE = "/api/column/rich/page"
    ...
    public static async httpCreate(authorName: string, columnName: string, columnPrice: number): Promise<Column> {
        ...
        let data = await BaseService.httpPost(ColumnService.URL_OMNIBUS, {
            authorName,
            columnName,
            columnPrice
        })
     ...
     * 获取富专栏列表
     */
    public static async httpRichPage(params: any): Promise<Pager<RichColumn>> {

        let data = await BaseService.httpGet(ColumnService.URL_RICH_PAGE, params)
